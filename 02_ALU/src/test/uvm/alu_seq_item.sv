// ADS I Class Project
// Assignment 02: Arithmetic Logic Unit and UVM Testbench
//
// Chair of Electronic Design Automation, RPTU University Kaiserslautern-Landau
// File created on 09/21/2025 by Tharindu Samarakoon
// File updated on 10/31/2025 by Tobias Jauch

`include "uvm_macros.svh"
import uvm_pkg::*;
import alu_tb_config_pkg::*;

class alu_seq_item extends uvm_sequence_item;

    // -------------------------------------------------
    // Randomized transaction fields
    // -------------------------------------------------
    rand bit [31:0] operandA;
    rand bit [31:0] operandB;
    rand ALUOp      operation;

    // Expected result (calculated by scoreboard / reference model)
    bit [31:0] aluResult;

    // -------------------------------------------------
    // UVM factory registration and field automation
    // -------------------------------------------------
    `uvm_object_utils_begin(alu_seq_item)
        `uvm_field_int  (operandA,  UVM_DEFAULT)
        `uvm_field_int  (operandB,  UVM_DEFAULT)
        `uvm_field_enum (ALUOp, operation, UVM_DEFAULT)
        `uvm_field_int  (aluResult, UVM_DEFAULT)
    `uvm_object_utils_end

    // -------------------------------------------------
    // Constraints
    // -------------------------------------------------
    constraint aluOp_constraint {
        operation inside {
            ADD,
            SUB,
            AND,
            OR,
            XOR,
            SLL,
            SRL,
            SRA,
            SLT,
            SLTU,
            PASSB
        };
    }

    // -------------------------------------------------
    // Utility: Convert transaction to string
    // -------------------------------------------------
    virtual function string convert2str();
        return $sformatf(
            "operandA: 0x%08x, operandB: 0x%08x, operation: %0p, aluResult: 0x%08x",
            operandA,
            operandB,
            operation,
            aluResult
        );
    endfunction

    // -------------------------------------------------
    // Constructor
    // -------------------------------------------------
    function new(string name = "alu_seq_item");
        super.new(name);
    endfunction

endclass
